...
void *malloc(size_t size)
{
    ...
    while (!fit)
    {
        ...
        {
            rsod("KERNEL_HEAP_EXHAUSTED", "ALLOCATOR");
        }

        if (!heap_expand(size))
        {
            rsod("KERNEL_HEAP_EXPANSION_FAILED", "ALLOCATOR");
        }
        fit = find_fit(size);
    }
    if (!fit)
        rsod("MALLOC_CRITICAL_FAILURE", "ALLOCATOR");
...
void free(void *ptr)
{
    ...
    /* проверяем magic */
    if (h->magic != MAGIC)
        rsod("HEAP_CORRUPTION_INVALID_MAGIC", "ALLOCATOR");

    /* проверка на многократное освобождение */
    if (h->free)
        rsod("DOUBLE_FREE", "ALLOCATOR");
